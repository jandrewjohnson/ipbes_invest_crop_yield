...
def run_regressions_and_save_results(model, regression, dataframe, features_selection,
                                     results_df,parameters_dict=None,inputs = None,subset=None):
    ...
    if features_selection == 'all':
        ...
        results_df = results_df.append({'Model': model,
                                        ...
                                        'inputs' : inputs,
                                        'subset': subset},ignore_index=True)
...
resultsf_inputs_simplesubset = pd.DataFrame(columns=['Model','num_features','Features','MSE','R2','inputs'])
...
resultsf_inputs_simplesubset = run_regressions_and_save_results(model, xgbreg,
                                                                df[simple_subset_cols].drop(['log_altitude'],axis=1),
                                                   features_selection, resultsf_inputs_simplesubset,
                                                   inputs='without altitude',subset='simple subset')

resultsf_inputs_simplesubset = run_regressions_and_save_results(model, xgbreg,
                                                                df[simple_subset_cols].drop(['slope'],axis=1),
                                                   features_selection, resultsf_inputs_simplesubset,
                                                   inputs='without slope',subset='simple subset')

resultsf_inputs_simplesubset = run_regressions_and_save_results(model, xgbreg,
                                                                df[simple_subset_cols].drop(['lat'],axis=1),
                                                   features_selection, resultsf_inputs_simplesubset,
                                                   inputs='without lat',subset='simple subset')


resultsf_inputs_simplesubset = run_regressions_and_save_results(model, xgbreg,
                                                                df[simple_subset_cols].drop(['sin_lon'],axis=1),
                                                   features_selection, resultsf_inputs_simplesubset,
                                                   inputs='without lon',subset='simple subset')

resultsf_inputs_simplesubset.to_csv('../ipbes_invest_crop_yield_project/intermediate/create_results_table/results_final_xgb3_inputs_geog_subset.csv')    
