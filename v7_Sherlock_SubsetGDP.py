...
def run_regressions_and_save_results(model, regression, dataframe, features_selection, results_df,parameters_dict=None):
...
# results_subset with best_parameters3
results_subset = pd.DataFrame(columns=['Model','num_features','Features','MSE','R2'])

model = 'XGB'
xgbreg = xgb.XGBRegressor(**best_parameters3)

for features_selection in ['all', 'RFE']:    
    results_subset = run_regressions_and_save_results(model, xgbreg, df[simple_subset_cols_gdp], features_selection, results_subset)
    
results_subset.to_csv('../ipbes_invest_crop_yield_project/intermediate/create_results_table/results_final_xgb3_mysubset_GDP.csv')    
